...
class BreitbartArticleInformation: 
    ...
    def dateCleaning(self, date):
        removeYear = date.replace(', 2020', '')
        splitOnSpace = removeYear.split(' ')
        dummyList = []
        dummyList.append(splitOnSpace[1])
        dummyList.append(splitOnSpace[0])
        finalDate = ' '.join(dummyList)
        return finalDate
